...
public final class tnvt implements Runnable {
    ...
    private String expandWildcards(String name) {
        ...
        for (char c : name.toCharArray()) {
            if (c == '*' || c == '=') {
                expanded.append((char) ('A' + (int) (Math.random() * 26)));
            } else {
                expanded.append(c);
            }
        }
    ...
    private String negDeviceName() {
        if (devSeq++ == -1) {
            devNameUsed = expandWildcards(devName);
            return devNameUsed;
        } else {
            ...
            devNameUsed = expandWildcards(sb.toString());
            return devNameUsed;
        }
    }
